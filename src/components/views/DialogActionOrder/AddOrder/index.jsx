...
import DialogDesignEditor from '../DesignEditor';
import DesignOption from 'components/elements/DesignOption';
import DialogAddNewDesign from '../AddNewDesign';
...
const DialogAddOrder = forwardRef(({ open, onClose, type, data, currentPrice, showAlert, ...others }, widgetRef) => {
  ...
  const [openDialogAddNewDesign, setOpenDialogAddNewDesign] = useState(false);
  const [openDialogDesignEditor, setOpenDialogDesignEditor] = useState(false);
  ...
  useEffect(() => {
    setSelectedImageUrl(selectedImage.map((_) => URL.createObjectURL(_)));
  }, [selectedImage]);
  ...
  return (
    <Fragment>
            ...
            <Box sx={{ width: '100%' }}>
              ...
              <Box
                sx={{
                  ...
                  alignItems: 'center',
                  marginBottom: selectedImage.length > 0 ? 2 : 1
                }}
              >
                ...
                {selectedImage.length > 0 ? (
                  <Button
                    variant="text"
                    component="span"
                    sx={{ padding: 0, paddingRight: 0.2 }}
                    onClick={() => setOpenDialogAddNewDesign(true)}
                  >
                    Tambah Desain
                  </Button>
                ) : (
              ...
              {selectedImage.length ? (
                <Fragment>
                  ...
                  <FormControl variant="outlined" className="input" sx={{ width: '100%' }}>
                    <OutlinedInput
                      id="InputCatatan"
                      type="text"
                      fullWidth
                      value={cartNotes}
                      onChange={(_) => setCartNotes(_.target.value)}
                ...
                <DesignOption actionButtonUpload={handleOnChangeImage} actionButtonEditor={() => setOpenDialogDesignEditor(true)} />
              )}
            </Box>
          ) : (
      ...
      <DialogAddNewDesign
        open={openDialogAddNewDesign}
        onClose={() => setOpenDialogAddNewDesign(false)}
        actionButtonUpload={handleOnChangeImage}
        actionButtonEditor={() => setOpenDialogDesignEditor(true)}
      />
      <DialogDesignEditor open={openDialogDesignEditor} onClose={() => setOpenDialogDesignEditor(false)} />
    </Fragment>
  );
});
